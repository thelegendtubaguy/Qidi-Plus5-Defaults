...
class LookAheadQueue:
    ...
    def add_move(self, move):
        ...
        #         move.smooth_delta_v2 = min(move.smooth_delta_v2, move.delta_v2) 
        #         if self.queue: 
        #             prev = self.queue[-1] # 只有 prev 还没被 flush 掉，才能修改它 
        #             if prev.accel > 500: # 保护正常直线段 
        #                 prev.accel = 500.0 # 上一段也降速，让过渡不那么剧烈 
        #                 prev.delta_v2 = 2.0 * prev.move_d * prev.accel 
        #                 prev.smooth_delta_v2 = min(prev.smooth_delta_v2, prev.delta_v2)
...
class ToolHead:
    def __init__(self, config):
        ...
        self.max_accel_cap = self.max_accel
        ...
        gcode.register_command('SET_EXTRUDER_ENABLE', self.cmd_SET_EXTRUDER_ENABLE)
        gcode.register_command('SET_EXTRUDER_E_ACCUMULATOR', self.cmd_SET_EXTRUDER_E_ACCUMULATOR)
        ...
        self.e_enable = False
        self.e_d = 0.01 #默认值为0.01

    def cmd_SET_EXTRUDER_E_ACCUMULATOR(self, gcmd):
        self.e_d = gcmd.get_float("D", self.e_d)
        gcmd.respond_info(f"Extruder distance set to: {self.e_d}")
    
    def cmd_SET_EXTRUDER_ENABLE(self, gcmd):
        enable = gcmd.get_int('ENABLE', 0)
        self.e_enable = bool(enable)
        gcmd.respond_info(f"Extruder enable set to: {self.e_enable}")
    ...
    def move(self, newpos, speed):
        if not self.e_enable:
            self._do_actual_move(newpos, speed)
        
        de = newpos[3] - self.commanded_pos[3]
        
        # 分支1：大幅度移动，直接执行，清空历史累积（避免溢出或压力提前干扰）
        if de == 0 or abs(de) >= 1.0:
            self.e_accumulator = 0.0
            self._do_actual_move(newpos, speed)
            return
        ...
        # 分支2：极小量，仅累积
        if abs(de) < self.e_d:
            self.e_accumulator += de
            if abs(self.e_accumulator) < self.e_d:
                return

        # 分支3：常规移动，合并累积量并执行
        target_pos = list(newpos)
        target_pos[3] += self.e_accumulator
        self.e_accumulator = 0.0
        
        self._do_actual_move(target_pos, speed)

    def _do_actual_move(self, newpos, speed):
    ...
    def cmd_SET_VELOCITY_LIMIT(self, gcmd):
        ...
        if max_accel is not None:
            ...
            self.max_accel = min(max_accel, self.max_accel_cap)
    ...
    def cmd_M204(self, gcmd):
        ...
        self.max_accel = min(accel, self.max_accel_cap)
        self._calc_junction_deviation()
