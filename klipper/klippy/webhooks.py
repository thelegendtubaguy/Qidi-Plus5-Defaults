...
# 定义深层匹配
DEEP_COMPARISON_MODULES = ["multi_color_controller"]
...
class QueryStatusHelper:
    ...
    def _do_query(self, eventtime):
        ...
        for cconn, subscription, send_func, template in msglist:
            ...
            for obj_name, req_items in subscription.items():
                ...
                for ri in req_items:
                    rd = res.get(ri, None)
                    last_val = lres.get(ri, None)
                    
                    if is_query:
                        # 新连接，全量更新
                        cres[ri] = rd
                    elif obj_name in DEEP_COMPARISON_MODULES:
                        # 特判模块：执行深度比对，只提取变化的部分
                        diff_result = self._get_diff_recursive(rd, last_val)
                        if diff_result is not None:
                            cres[ri] = diff_result
                    elif rd != last_val:
                        # 普通模块：保持原有的浅层比对（Level 2 变化则全量发 Level 2）
                        cres[ri] = rd
                    # 原逻辑
                    # rd = res.get(ri, None)
    ...
    def _get_diff_recursive(self, now, last):
        """递归获取两个对象的差异内容"""
        if not isinstance(now, dict) or not isinstance(last, dict):
            return now if now != last else None
        ...
        for k, v in now.items():
            if k not in last:
                ...
                res = self._get_diff_recursive(v, last[k])
                if res is not None:
                    diff[k] = res
