...
class PrinterRail:
    def __init__(self, config, need_position_minmax=True,
        ...
        self.homing_tolerance = config.getfloat(
            'homing_tolerance', 0.03, minval=0.01)
        self.homing_retries = config.getint(
            'homing_retries', 2, minval=0)
    ...
    def get_homing_info(self):
        homing_info = collections.namedtuple('homing_info', [
            ...
            'tolerance', 'retries'])(
                self.homing_speed, self.position_endstop, self.homing_retract_speed, self.homing_retract_dist,
                self.homing_positive_dir, self.second_homing_speed,
                self.homing_tolerance, self.homing_retries)
        return homing_info
