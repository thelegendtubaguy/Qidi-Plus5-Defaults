...
class _IRQButtonMCU:
    ...
    def _build_config(self):
        ...
        self.query_cmd = self.mcu.lookup_command(
            "irq_button_query oid=%c", cq=cmd_queue)
    ...
    #     count = msg["count"]
    #     states = msg["state"]
    #     logging.warning("IRQ button state oid=%d count=%d states=%s self.state=%d",
    #                     self.oid, count, states, self.state)
    #     for s in states:
    #         s = int(s) ^ self.invert
    #         s1 = s ^ self.invert
    #         logging.warning("sample: s0=%d -> s1=%d", s, s1)
    #         if s != self.state:
    ...
    def _handle_state(self, msg):
        ...
        logging.warning("IRQ button state oid=%d count=%d states=%s self.state=%d",
                         self.oid, count, states, self.state)
        for s in states:
            value = int(s) ^ (self.invert & 1)
            self.reactor.register_async_callback(
                lambda et, c=self.callbacks[0][2], v=value: c(et, v))
        self.ack_cmd.send([self.oid, count])
...
class MCU_irq_button:
    ...
    def register_buttons(self, pins, callback, debounce_us, share=True):
       ...
       #  share=False：每次新建一个 _IRQButtonMCU（单 pin 最稳）
        if not share:
            mcu_button = _IRQButtonMCU(self.printer, mcu, debounce_us)
            # 用唯一 key 保存，防止被覆盖/回收
            key = f"{mcu_name}:{mcu_button.oid}"
            self.irq_buttons[key] = mcu_button
            mcu_button.setup_buttons(pin_params_list, callback)
            return mcu_button

        #  share=True：沿用原逻辑（最多8路复用）
