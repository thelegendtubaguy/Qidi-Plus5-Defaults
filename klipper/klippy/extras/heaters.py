...
class Heater:
    ...
    def temperature_callback(self, read_time, temp):
        ...
        try:
            ...
            if position_z > self.z_max_limit:
...
class ControlPID:
    def __init__(self, heater, config):
        ...
        # === 新增：用于启动限流的变量 ===
        self.last_bounded_co = 0.          # 上一次实际输出的功率
        self.start_time = None              # 当前加热周期的启动时间（None 表示未加热）
        # ==============================
    ...
    def temperature_update(self, read_time, temp, target_temp):
        ...
        # === 新增：腔室加热器启动限流（前5秒限制为30%功率） ===
        if self.heater.name == "heater_generic chamber":
            now = read_time
            # 检测是否开始加热（输出功率从0变为正）
            if bounded_co > 0 and self.last_bounded_co <= 0:
                self.start_time = now
            elif bounded_co <= 0:
                # 加热器关闭，重置启动时间
            ...
            # 如果在启动后的5秒内，限制最大功率
            if self.start_time is not None and (now - self.start_time) < 5.0:
                limit_power = self.heater_max_power * 0.3   # 30% 限制
                if bounded_co > limit_power:
                    bounded_co = limit_power
        # 保存本次输出值，供下次判断使用
        self.last_bounded_co = bounded_co
        # ==================================================
