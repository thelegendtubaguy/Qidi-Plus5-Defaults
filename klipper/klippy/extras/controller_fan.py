...
class ControllerFan:
    ...
    def callback(self, eventtime):
        ...
        #qidi-0311
        if self.fan.rpm_error == 1:
            self.fan.rpm_error = 2

            msg = 'Code:QDE_002_002;Message:{"fan_name":"%s"} speed check error ,turn off {"heater":"%s"}' % (self.fan.name,self.heater_names)
            if heater.get_name().split()[-1] == "chamber":            
                heater.set_temp(0)
